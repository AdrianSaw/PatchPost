...
- **CI:** every PR runs full `npm test` against an ephemeral local Supabase stack started in GitHub Actions (Docker on `ubuntu-latest`); keys come from `npx supabase status`, not hosted secrets. Local dev is unchanged — use `.env.local` with your own Docker Supabase.
- Playwright E2E is planned per @README_PatchPost_plan.md — not wired yet.
