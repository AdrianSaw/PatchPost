...
| F-02 | project-and-draft-data-foundation | (foundation) warstwa danych wspiera projekty, źródła zmian i trwały zapis draftów | — | FR-002, FR-005 | ready |
...
- **Data:** partial — klient Supabase (`src/lib/supabase.ts`, `supabase/config.toml`); brak migracji domenowych, tabel i RLS produktowych.
- **Auth:** partial — logowanie Supabase + middleware działają; **publiczny signup nadal włączony**; invite-only = użytkownicy tylko z Supabase Auth (dashboard), bez osobnej listy email w env aplikacji.
...
| F-02 | project-and-draft-data-foundation | Establish project and draft data lifecycle | yes | Równolegle z S-01/F-01; wymagane przed S-02/S-03 |
| S-01 | invite-only-signin-gated-access | Enforce invite-only signin and remove public signup | yes | Run `/10x-plan invite-only-signin-gated-access` — pierwszy ruch |
